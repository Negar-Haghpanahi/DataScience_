...
class Cluster():
    ...
    def calculate(self , model , list , target):
        ...
        if model == inputtype.clustering.k_prototype:
            ...
            self.df=ReadingFile.Preparation(self.df).addColumn(self.df , target , self.y)
            self.df=ReadingFile.Preparation(self.df).addColumn(self.df ,'cluster' , cluster_dict)
            self.df=ReadingFile.Preparation(self.df).addColumn(self.df ,'cluster_0' , 5) 
            self.df=ReadingFile.Preparation(self.df).addColumn(self.df ,'cluster_1' , 6)
            self.df=ReadingFile.Preparation(self.df).addColumn(self.df ,'cluster_2' , 7)
            print("cluster")
            print(self.df)
